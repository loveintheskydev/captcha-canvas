...
export class Captcha {
  ...
  constructor(
    ...
    characters: number = defaultCaptchaOption.characters ?? 6,
  ) {
    this._height = height;
    this._width = width;
    ...
    if (Array.isArray(captchaOption)) {
      ...
      for (const option of captchaOption) {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          if (option.skew) {
            this._ctx.transform(1, Math.random(), getRandom(20) / 100, 1, 0, 0);
          }
          if (option.rotate && option.rotate > 0) {
            this._ctx.rotate((getRandom(-option.rotate, option.rotate) * Math.PI) / 180);
          }
          if (option.colors && option.colors?.length >= 2) {
            this._ctx.fillStyle = option.colors[getRandom(option.colors.length - 1)];
          }
      ...
      if (!this._coordinates[0])
        this._coordinates = getRandomCoordinate(this._height, this._width, option.characters || 6);
      const coordinates: number[][] = this._coordinates;
